...
// File @zilionixx/zilionixx-swap-lib/contracts/math/SafeMath.sol@v1.0.1
...
// File @zilionixx/zilionixx-swap-lib/contracts/token/BEP20/IBEP20.sol@v1.0.1
...
// File @zilionixx/zilionixx-swap-lib/contracts/utils/Address.sol@v1.0.1
...
// File @zilionixx/zilionixx-swap-lib/contracts/token/BEP20/SafeBEP20.sol@v1.0.1
...
// File @zilionixx/zilionixx-swap-lib/contracts/GSN/Context.sol@v1.0.1
...
// File @zilionixx/zilionixx-swap-lib/contracts/access/Ownable.sol@v1.0.1
...
// File @zilionixx/zilionixx-swap-lib/contracts/token/BEP20/BEP20.sol@v1.0.1
...
// File contracts/ZilionixxToken.sol

pragma solidity =0.6.6;

contract ZilionixxToken is BEP20('ZilionixxToken', 'Cola') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (ZilionixxMaster).
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), 'GRIMEX::delegateBySig: invalid signature');
        require(nonce == nonces[signatory]++, 'GRIMEX::delegateBySig: invalid nonce');
        require(now <= expiry, 'GRIMEX::delegateBySig: signature expired');
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, 'GRIMEX::getPriorVotes: not yet determined');
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying GRIMEXs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, 'GRIMEX::_writeCheckpoint: block number exceeds 32 bits');
...
// File contracts/ZilionixxMaster.sol
...
contract ZilionixxMaster is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GRIMEXs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (userInfo.amount * pool.accGrimexPerShare) - userInfo.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accGrimexPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // How many allocation points assigned to this pool. GRIMEXs to distribute per block.
        uint256 lastRewardBlock; // Last block number that GRIMEXs distribution occurs.
        uint256 accGrimexPerShare; // Accumulated GRIMEXs per share, times 1e12. See below.
        bool exists; //
    }
    // GRIMEX tokens created first block.
    uint256 public grimexStartBlock;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when GRIMEX mining starts.
    uint256 public startBlock;
    // Block number when bonus GRIMEX period ends.
    ...
    // GRIMEX tokens created at bonus end block.
    uint256 public grimexBonusEndBlock;
    // max reward block
    ...
    // Accumulated GRIMEXs per share, times 1e12.
    uint256 public accGrimexPerShareMultiple = 1E12;
    // The GRIMEX TOKEN!
    ZilionixxToken public grimex;
    // Dev address.
    ...
    constructor(
        ZilionixxToken _grimex,
        address _devAddr,
        uint256 _grimexStartBlock,
        uint256 _startBlock,
        ...
        grimex = _grimex;
        devAddr = _devAddr;
        grimexStartBlock = _grimexStartBlock;
        ...
        // grimex created when bonus end first block.
        // (grimexStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        grimexBonusEndBlock = grimexStartBlock
        ...
        // max mint block number, _grimexInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_grimexInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(grimexBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function add(
        ...
        pool.accGrimexPerShare = 0;
        pool.exists = true;
        poolAddresses.push(_pair);
    }

    // Update the given pool's GRIMEX allocation point. Can only be called by the owner.
    function set(
        address _pair,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_grimexInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            totalReward = tempTo
                ...
                .mul(_grimexInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _grimexInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempGrimexInitBlock = _grimexInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(
                            tempGrimexInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference))
                        )
                    );
                }
            }
    ...
    // View function to see pending GRIMEXs on frontend.
    function pendingGrimex(address _pair, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfoMap[_pair];
        if (!pool.exists) {
            return 0;
        }
        UserInfo storage userInfo = poolUserInfoMap[_pair][_user];
        uint256 accGrimexPerShare = pool.accGrimexPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 grimexReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            accGrimexPerShare = accGrimexPerShare.add(grimexReward.mul(accGrimexPerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(accGrimexPerShare).div(accGrimexPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 grimexReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        grimex.mintTo(devAddr, grimexReward.div(100));
        grimex.mintTo(address(this), grimexReward);
        pool.accGrimexPerShare = pool.accGrimexPerShare.add(grimexReward.mul(accGrimexPerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to ZilionixxMaster for GRIMEX allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.accGrimexPerShare).div(accGrimexPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
                safeGrimexTransfer(msg.sender, pending);
            }
        }
        IBEP20(_pair).safeTransferFrom(address(msg.sender), address(this), _amount);
        userInfo.amount = userInfo.amount.add(_amount);
        userInfo.rewardDebt = userInfo.amount.mul(pool.accGrimexPerShare).div(accGrimexPerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from ZilionixxMaster.
    ...
    // Safe grimex transfer function, just in case if rounding error causes pool to not have enough GRIMEXs.
    function safeGrimexTransfer(address _to, uint256 _amount) internal {
        uint256 grimexBal = grimex.balanceOf(address(this));
        if (_amount > grimexBal) {
            grimex.transfer(_to, grimexBal);
        } else {
            grimex.transfer(_to, _amount);
        }
    }
    ...
    // Update tokenOwnerShip
    function updateTokenOwnerShip(address ownerAddr) public onlyOwner {
        grimex.transferOwnership(ownerAddr);
    }
}
